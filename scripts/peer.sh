...
if (( argc != 9 )) 
then
	echo "Usage: $0 <version> <peer_ap> <peer_id> <mc_addr> <mc_port> <mdb_addr> <mdb_port> <mdr_addr> <mdr_port>"
	exit 1
fi
...
if [[ (( $ver != '1.0' )) && (( $ver != '1.1' )) ]]
then
  echo "Unknown Protocol Version. Known Versions: 1.0 and 1.1"
  exit 1
fi
...
# echo "java peer.Peer ${ver} ${pap} ${id} ${mc_addr} ${mc_port} ${mdb_addr} ${mdb_port} ${mdr_addr} ${mdr_port}"

java Peer ${ver} ${pap} ${id} ${mc_addr} ${mc_port} ${mdb_addr} ${mdb_port} ${mdr_addr} ${mdr_port}
